...
In essence, the hourglass model states that defining minimal standards (protocols) at the waist of the hourglass is crucial towards supporting a wide range of applications and supporting services at the top and bottom, respectively, of the hourglass. For the internet, the IP protocal constitutes the centre of the hourglass. Although the IP protocol is very limited in itself (it can only send and receive data packets), it is exactly because of its minimal standard that the IP protocal is able to support a great diversity of applications on top of it (web apps, email, videoconferencing) and allow implementation using a great diversity of supporting services below it (in this case, the many different types of physical networks). Despite this flexibility and extensiblity, however, the hourglass model strictly states that the components that lie above the central layer cannot directly access the services that lie below it.

![The hourglass model for health data](./docs/images/fhir-data-stations.png){#fig-fhir-data-stations}

The hourglass model provides an effective and practical paradigm to achieve interoperability for health data, too, as is shown in @fig-fhir-data-stations. Essentially, we propose an amalgamation of the design principles of OpenHIE, Fast Healthcare Interoperability Resources (FHIR) and FAIR to make data Findable, Accessible, Interoperable and Reusable.
...
- __Shared Patient Record in Bulk FHIR format__: data from all patients and participating health facilities in the MomCare programme are persisted as Shared Patient Records using the Bulk FHIR data format. While the FHIR standard was originally designed as an interoperability standard for information exchange, we demonstrate its use for persistent data storage as means for realizing the Shared Health Record component. We demonstrate that the data can be stored and made accessible using standard cloud storage technologies to enable secondary re-use of bulk data through federated analytics;
- __Business Services__:
  - __Automated DHIS2 reporting__: existing management reports can be reproduced and generated from the FHIR-transformed data, thereby reducing double entry and opening up possibilities improvement management reporting;
  - __Value-Points Dashbboards__: value-based care models and conditional payment schemes can be supported using the FHIR-transformed data. This opens up possibilites of scaling up innovative care models withou the necessity of deploying the MomCare app per se. Provided that the data required for running the value-based care programme is already registered in the Point-of-Service system (an app or EMR), the data can be re-used;
...
Above mentioned demonstrators have been build using open source technologies and implemented using standard cloud technologies. A schematic overview is shown in @fig-openhie-momcare-tz. Within the setting of the MomCare project, Azure was chosen as the cloud infrastructure on which the components have been deployed, but the components can be deployed on any cloud hosting provider.
